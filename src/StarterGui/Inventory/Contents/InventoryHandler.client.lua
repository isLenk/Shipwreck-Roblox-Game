...
local dropBox = script.Parent.BodyFrame.InventoryFrame:WaitForChild("DropBox")

local slotSeperator = slotsContainer:WaitForChild("UIGrid_SlotsContainer")

local rDrop = sharedDatabase.Remotes.FromClient:WaitForChild("DropInventoryItem")
...
local function updateSlots(child, func)
	...
	local function modObjects()
		...
		elseif func == "remove" then
			for i = 1, #selectedCategory[3] do
				if typeof(selectedCategory[3][i]) == "Instance" and selectedCategory[3][i].Name == child.Name then
					print("Removing", child.Name)
					selectedCategory[3][i] = 1
					print(selectedCategory[3][i])
					return
				end
			end
		end
	...
	local function updateFuncs()
		for objIndex = 1, #selectedCategory[3] do
			...
			elseif typeof(selectedCategory[3][objIndex]) == "number" then
