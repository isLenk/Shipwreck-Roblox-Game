...
local myLocalData = player.PlayerData.Value.LocalData
...
script.Parent:GetPropertyChangedSignal("Enabled"):Connect(function()
	myLocalData.MouseLocked.Value = not script.Parent.Enabled
end)

game:GetService("UserInputService").InputEnded:Connect(function(key)
	if (key.KeyCode == mod_hotkey:getKey("Inventory")) then --and not operating then
